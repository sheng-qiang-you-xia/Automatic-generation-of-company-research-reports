...
class IntegratedResearchReportGenerator:
    ...
    def stage1_data_collection(self):
        ...
        target_results = baidu_search(target_search_keywords)
        ...
        for company in listed_companies:
            ...
            competitor_results = baidu_search(search_keywords)
...
"""
        ...
        try:
            ...
            if isinstance(parts, dict):
                # 如果是字典，转换为列表
                parts = [{"part_title": k, "part_desc": v} if isinstance(v, str) else v for k, v in parts.items()]
            elif isinstance(parts, list):
                # 如果是列表，确保每个元素都是字典
                for i, part in enumerate(parts):
                    if isinstance(part, str):
                        parts[i] = {"part_title": part, "part_desc": f"{part}相关内容"}
                    elif not isinstance(part, dict):
                        parts[i] = {"part_title": f"部分{i+1}", "part_desc": str(part)}
            else:
                # 如果解析失败，使用默认大纲
                parts = [
                    {"part_title": "公司概况", "part_desc": "公司基本情况和业务介绍"},
                    {"part_title": "财务分析", "part_desc": "财务数据分析和指标解读"},
                    {"part_title": "行业对比", "part_desc": "与同行业公司的对比分析"},
                    {"part_title": "估值与预测", "part_desc": "公司估值分析和未来预测"},
                    {"part_title": "投资建议", "part_desc": "投资建议和风险提示"}
                ]
        ...
        print(f"\n===== 解析后的大纲结构 =====\n")
        for i, part in enumerate(parts):
            print(f"{i+1}. {part.get('part_title', '未知标题')}: {part.get('part_desc', '无描述')}")
...
"""
    ...
    def convert_to_docx(self, output_file, docx_output=None):
        ...
        if not pandoc_path:
            print(f"[提示] 未找到pandoc，跳过Word文档生成")
            print("[建议] 请安装pandoc:")
            print("  Ubuntu/Debian: sudo apt-get install pandoc")
            print("  CentOS/RHEL: sudo yum install pandoc")
            print("  macOS: brew install pandoc")
            print("  Windows: 下载 https://pandoc.org/installing.html")
            return
        ...
        try:
            ...
            print(f"使用pandoc路径: {pandoc_path}")
            result = subprocess.run(pandoc_cmd, check=True, capture_output=True, 
            ...
            print(f"[提示] pandoc转换失败: {e}")
            print(f"[建议] 请检查pandoc是否正确安装，或手动转换: {pandoc_path} {output_file} -o {docx_output}")
